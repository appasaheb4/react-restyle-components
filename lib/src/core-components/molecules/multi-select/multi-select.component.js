...
export const MultiSelect = ({ options = [], selectedItems = [], hasError = false, onSelect, }) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: `flex dark:bg-boxdark dark:text-white flex-col w-full rounded-md border-2 ${hasError ? 'border-red  ' : 'border-gray-300'}`, ref: wrapperRef },
            ...
            React.createElement("div", { className: `flex ${isListOpen ? `show` : `hidden`}` }, options
                ? options?.length > 0 && (React.createElement("ul", { className: "flex  flex-col gap-2  bg-gray-200 w-full" }, options?.map((item, index) => (React.createElement("li", { key: index, className: "flex gap-4 p-2" },
