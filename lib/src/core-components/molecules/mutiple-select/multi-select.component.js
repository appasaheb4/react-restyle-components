...
export const MultipleSelect = ({ options = [], selectedItems = [], hasError = false, onSelect, }) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: `flex dark:bg-boxdark dark:text-white flex-col w-full rounded-md border-2 ${hasError ? 'border-red  ' : 'border-gray-300'}`, ref: wrapperRef },
            ...
            React.createElement("div", { className: `flex mx-2 ${isListOpen ? `show` : `hidden`}` }, options
                ? options?.length > 0 && (React.createElement("ul", null, options?.map((item, index) => (React.createElement("li", { key: index, className: "flex items-center text-center" },
                    React.createElement("input", { className: "bg-black", type: "checkbox", checked: selectedOptions?.includes(item), onChange: () => {
                    ...
                    ' ',
                    React.createElement("label", { className: "ml-3 mt-2 pt-1 dark:text-white" },
                        ' ',
                        item))))))
                : null))));
};
