...
export const Autocomplete = ({ value = '', onValueChange, options = [], }) => {
    ...
    const renderList = () => {
        ...
        if (shouldShowList) {
            if (filteredOptions.length) {
                return (React.createElement("ul", { role: "list", className: "autocomplete-list w-full", id: "list", ref: listRef }, filteredOptions?.map((option, index) => {
                    let className = '';
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: `flex flex-col  leading-4 p-2 bg-white focus:outline-none focus:ring  w-full shadow-sm sm:text-base border-2 rounded-md` },
            React.createElement("input", { type: "text", role: "input", name: "autocomplete-input", className: "w-full focus:outline-none", placeholder: "Search...", onChange: onInputChange, onKeyDown: onKeyDown, value: searchValue, ref: inputRef, onClick: () => {
                    setShouldShowList(true);
                } }),
            renderList())));
};
