...
export const MultiSelectionDropdown = ({ options = [], onSelect, onClear, placeholder = 'Select Items', disabled = false, preSelected, hasError = false, }) => {
    ...
    return (_jsxs("div", { className: cn(s['relative']), ref: dropdownRef, children: [_jsxs("button", { type: "button", onClick: () => !disabled && setIsOpen(!isOpen), className: cn(s['w-full'], s['flex'], s['justify-between'], s['items-center'], s['px-3'], s['py-2'], s['border'], {
