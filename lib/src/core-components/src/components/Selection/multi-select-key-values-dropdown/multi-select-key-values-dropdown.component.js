...
/** Keys not copied from the parent into the nested `parent` bag on `SelectedData`. */
...
const mapItemsToPayload = (items, resolveFields) => items.map((item) => {
    ...
    const parentBag = item.parentDynamic;
    const hasParentExtras = parentBag && Object.keys(parentBag).length > 0;
    return {
        ...passthroughChildExtras(item),
        ...(hasParentExtras ? { parent: parentBag } : {}),
...
export const MultiSelectKeyValuesDropdown = ({ options = [], onSelect, onClear, placeholder = 'Select items', disabled = false, isDisable = false, hasError = false, getFieldsResolver, }) => {
    ...
    const toggleSelection = useCallback((parent, child) => {
        ...
        setSelectedItems((prev) => {
            ...
            const parentDynamic = passthroughParentExtras(parent);
            const hasParentDynamic = parentDynamic != null &&
                typeof parentDynamic === 'object' &&
                Object.keys(parentDynamic).length > 0;
            const next = isSelected
                ...
                : [
                    ...prev,
                    {
                        ...
                        selectionId: id,
                        ...(hasParentDynamic ? { parentDynamic } : {}),
                    },
                ];
            emitSelection(next);
            return next;
    ...
    return (_jsxs("div", { className: cn(s['relative']), ref: dropdownRef, children: [_jsxs("button", { type: "button", onClick: () => !isDisabled && setIsOpen(!isOpen), className: cn(s['w-full'], s['flex'], s['justify-between'], s['items-center'], s['px-3'], s['py-2'], s['border'], {
                            ...
                            return (_jsxs(React.Fragment, { children: [_jsx("li", { className: cn(s['flex'], s['items-center']), style: {
                                        ...
                                        return (_jsxs("li", { onClick: () => toggleSelection(parent, child), className: cn(s['flex'], s['items-center'], s['cursor-pointer']), style: {
                                                ...
                                                e.currentTarget.style.backgroundColor = isChecked
                                                    ? 'rgb(239, 246, 255)'
                                                    : 'transparent';
                                            }, children: [_jsx("input", { type: "checkbox", checked: isChecked, onChange: () => toggleSelection(parent, child), onClick: (e) => e.stopPropagation(), style: {
                                                        width: '16px',
                                                        height: '16px',
                                                        accentColor: '#3b82f6',
                                                        cursor: 'pointer',
