...
const buildFieldLimitMap = (limitText) => {
    if (!limitText?.length)
        return {};
    return limitText.reduce((acc, entry) => {
        Object.entries(entry).forEach(([key, limit]) => {
...
const formatFieldDisplayValue = (rawValue, key, fieldLimits, applyLimit) => {
    if (rawValue === null || rawValue === undefined)
        return null;
    const text = String(rawValue);
    const limit = fieldLimits[key];
    if (applyLimit && limit != null && text.length > limit) {
        return `${text.slice(0, limit)}...`;
    }
    return text;
};
const formatItemDisplayText = (item, displayKey, limitText, applyLimit = true) => {
    const fieldLimits = buildFieldLimitMap(limitText);
    return displayKey
        .map((key) => formatFieldDisplayValue(item[key], key, fieldLimits, applyLimit))
...
export const AutoCompleteFilterSingleSelectMultiFieldsDisplay = ({ disable = false, loader = false, displayValue = '', placeholder = 'Search...', data, hasError = false, className, posstion = 'absolute', keyboard = 'text', filterDebounceDelay = 1000, classNames: customClassNames = {}, styles: customStyles = {}, onFilter, onSelect, onBlur, }) => {
    ...
    const onKeyDown = (e) => {
        if (e.key === 'Enter' || e.key === 'Tab') {
            if (options.length > 0) {
                ...
                if (selectedItem) {
                    setValue(formatItemDisplayText(selectedItem, data.displayKey, data.limitText, true));
                    handleCloseDropdown();
    ...
    const handleSelectItem = useCallback((item) => {
        setValue(formatItemDisplayText(item, data.displayKey, data.limitText, true));
        handleCloseDropdown();
        onSelect && onSelect(item);
    }, [data.displayKey, data.limitText, onSelect, handleCloseDropdown]);
    return (_jsx(_Fragment, { children: _jsxs("div", { ref: wrapperRef, className: cn(s['w-full'], s['relative'], customClassNames.container), style: customStyles.container, children: [_jsxs("div", { className: cn(s['flex'], s['p-2'], s['leading-4'], s['focus:outline-none'], s['focus:ring'], s['w-full'], s['shadow-sm'], s['sm:text-base'], s['border'], s['rounded-md'], s['transition-all'], s['duration-200'], {
                        ...
                        ...customStyles.inputWrapper,
                                ...
                                alignItems: 'center',
                                gap: '8px',
                            }, children: [isFiltering && _jsx(LoadingSpinner, {}), _jsx("div", { style: {
                            ...
                            ...(isAnimating
                                ...
                                : animationStyles.fadeContent.ready),
                        }, children: options.length > 0 ? (_jsx("ul", { style: { listStyle: 'none', margin: 0, padding: 0 }, children: options.map((item, index) => (_jsx("li", { className: cn(s['flex'], s['items-center']), onClick: () => handleSelectItem(item), onMouseEnter: () => setHoveredIndex(index), onMouseLeave: () => setHoveredIndex(null), style: {
                                    ...
                                    transitionDelay: `${Math.min(index * 20, 150)}ms`,
                                        ...
                                        cursor: 'pointer',
                                        fontSize: '14px',
                                    }, title: formatItemDisplayText(item, data.displayKey, data.limitText, false), children: formatItemDisplayText(item, data.displayKey, data.limitText, true) }) }, index))) })) : (_jsx(AutoCompleteEmptyState, { loader: loader || isFiltering, hasSearchText: !!value?.trim(), loadingDelayMs: filterDebounceDelay, emptyStateMessage: "No Results Found", emptyStateDescription: "Try adjusting your search" })) }) }))] }) }));
};
