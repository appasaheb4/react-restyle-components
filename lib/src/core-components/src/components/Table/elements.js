...
export const Toolbar = styled.div `
  ...
  /* Single-row toolbar: scroll horizontally on narrow screens instead of wrapping */
  & > * {
    flex-shrink: 0;
  }
...
`;
export const SelectSortButton = styled.button `
  display: inline-flex;
  align-items: center;
  justify-content: center;
  width: 32px;
...
export const ColumnTogglePanel = styled.div `
  ...
  top: ${({ $fixed, $top }) => $fixed && $top != null ? `${$top}px` : $fixed ? 'auto' : '100%'};
  left: ${({ $fixed, $left }) => $fixed && $left != null ? `${$left}px` : 'auto'};
