...
import React, { forwardRef, useState, useCallback, useMemo, useEffect, useLayoutEffect, useRef, } from 'react';
...
import { useSortState, useFilterState, usePaginationState, useRowSelection, useRowExpansion, useColumnVisibility, useTableDebounce, sortData, filterData, paginateData, getNestedValue, exportToCSV, exportToExcel, applyCellDefault, } from './hooks';
import { getFilterComponent, createCallableFilterHandle, FilterValueProvider, FilterClickApplyProvider } from './filters';
...
// Checkmark Icon for selection indicator
const CheckIcon = ({ size = 16, color = 'white', }) => (_jsx("svg", { viewBox: "0 0 24 24", fill: "none", stroke: color, strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round", style: { width: size, height: size, display: 'block' }, children: _jsx("path", { d: "M20 6L9 17l-5-5" }) }));
...
export const Table = forwardRef(function TableComponent(props, ref) {
    ...
    const resolvedRemote = useMemo(() => {
        ...
        // If totalSize is not provided, we have all data client-side - use client-side
        // (sort, filter, pagination work locally; otherwise sort/filter would never apply)
        if (totalSize === undefined) {
            return false;
        }
    ...
    const [searchDraft, setSearchDraft] = useState(cachedFilterState?.searchValue ?? defaultSearchValue);
    const searchDraftRef = useRef(cachedFilterState?.searchValue ?? defaultSearchValue);
    ...
    const [columnTogglePosition, setColumnTogglePosition] = useState({
        top: 0,
        left: 0,
    });
    ...
    const COLUMN_TOGGLE_PANEL_WIDTH = 280;
    const updateColumnTogglePosition = useCallback(() => {
        const anchor = columnToggleRef.current;
        if (!anchor)
            return;
        const rect = anchor.getBoundingClientRect();
        const left = Math.min(Math.max(8, rect.right - COLUMN_TOGGLE_PANEL_WIDTH), window.innerWidth - COLUMN_TOGGLE_PANEL_WIDTH - 8);
        setColumnTogglePosition({
            top: rect.bottom + 4,
            left,
        });
    }, []);
    // Position column toggle panel with fixed coords so it isn't clipped by toolbar overflow
    useLayoutEffect(() => {
        if (!columnToggleOpen)
            return;
        updateColumnTogglePosition();
        window.addEventListener('resize', updateColumnTogglePosition);
        window.addEventListener('scroll', updateColumnTogglePosition, true);
        return () => {
            window.removeEventListener('resize', updateColumnTogglePosition);
            window.removeEventListener('scroll', updateColumnTogglePosition, true);
        };
    }, [columnToggleOpen, updateColumnTogglePosition]);
    ...
    const clickSearchApply = Boolean(isClickFilter && searchable);
    useEffect(() => {
        if (!clickSearchApply)
            return;
        setSearchDraft(searchValue);
    }, [searchValue, clickSearchApply]);
    ...
    const invokeRemoteOnFilter = useCallback((type, filtersOverride, searchOverride) => {
        if (!onFilterRef.current)
            return;
        const sourceFilters = filtersOverride ?? filters;
        const searchVal = searchOverride ?? debouncedSearchValue;
        const cleanFilters = buildCleanFilters(sourceFilters);
        const filterData = { ...cleanFilters };
        if (type === 'search' && searchVal) {
            filterData.srText = searchVal;
        }
        onFilterRef.current(type, filterData, page + 1, pageSize);
    }, [filters, debouncedSearchValue, page, pageSize, buildCleanFilters]);
    ...
    // Resolved key field for row identification
    const resolvedKeyField = resolvedRowSelection?.keyField || keyField || '_id';
    // Extract row selection mode for use in dependencies
    const rowSelectionMode = resolvedRowSelection?.mode || 'none';
    ...
    const processedData = useMemo(() => {
        ...
        // Sort: always apply when sort state is set (works for both client-side and remote)
        // sortData returns data unchanged when !sort.field || !sort.order
        result = sortData(result, sort, columns);
        // Select-sort: move selected rows to top (when user clicked the button)
        // This applies in both remote and client-side modes
        // Only apply if isSelectSort is enabled AND row selection is enabled AND we have keys to sort
        if (isSelectSort &&
            (rowSelectionMode === 'checkbox' || isSelectRow) &&
            selectSortKeys.length > 0 &&
            resolvedKeyField) {
            const sortKeysSet = new Set(selectSortKeys);
            const selected = [];
            const notSelected = [];
            result.forEach((row) => {
                const rowKeyValue = getNestedValue(row, resolvedKeyField);
                const rowKey = rowKeyValue != null ? String(rowKeyValue) : '';
                if (rowKey && sortKeysSet.has(rowKey)) {
                    selected.push(row);
                }
                else {
        ...
        // Paginate (only for client-side)
        if (!resolvedRemote && pagination) {
    ...
    // Handle search
    const handleSearchChange = useCallback((value, options) => {
        resetEditingState();
        setInternalSearchValue(value);
        onSearch?.(value);
        filterTypeRef.current = 'search';
        if (options?.clickApply && onFilter) {
            // Enter-to-apply: invoke on every apply, even when the search text is unchanged
            invokeRemoteOnFilter('search', filters, value);
        ...
        saveFilterCache,
        onFilter,
        invokeRemoteOnFilter,
        filters,
    ]);
    const handleGlobalSearchInputChange = useCallback((value) => {
        if (clickSearchApply) {
            setSearchDraft(value);
            return;
        }
        handleSearchChange(value);
    }, [clickSearchApply, handleSearchChange]);
    const commitGlobalSearch = useCallback(() => {
        handleSearchChange(searchDraftRef.current, { clickApply: clickSearchApply });
    }, [handleSearchChange, clickSearchApply]);
    // Handle sort
    ...
    }, [sort, handleSort, onSortChange, onChange, resetEditingState]);
    // Handle filter
    const handleFilterChange = useCallback((field, value, options) => {
        resetEditingState();
        setFilter(field, value);
        const newFilters = { ...filters, [field]: value };
        const filtersForRemote = value === undefined || value === null || value === ''
            ? Object.fromEntries(Object.entries(filters).filter(([key]) => key !== field))
            : newFilters;
        onFilterChange?.(newFilters);
        filterTypeRef.current = 'filter';
        if (options?.clickApply && onFilter) {
            // Enter-to-apply / filterRenderer click: invoke on every apply, even when value is unchanged
            invokeRemoteOnFilter('filter', filtersForRemote);
            prevFiltersRef.current = JSON.stringify(filtersForRemote);
            shouldCallOnFilter.current = false;
        }
        else {
            shouldCallOnFilter.current = true;
    ...
    const handleRowClick = useCallback((row, rowIndex, e) => {
        ...
        if (checkboxProps?.disabled && !checkboxProps?.checkboxOnly) {
            return;
        }
    ...
    const handleCellDoubleClick = useCallback((row, rowIndex, column, colIndex, e) => {
        ...
        if (resolvedRowSelection) {
            const checkboxProps = resolvedRowSelection.getCheckboxProps?.(row);
            const isRowDisabled = !!checkboxProps?.disabled && !checkboxProps?.checkboxOnly;
    ...
    const handleExport = useCallback(async () => {
        ...
        const enhancedColumns = exportColumns.map((col) => {
            ...
            if (config?.type) {
                return {
                    ...col,
                    csvFormatter: (cell, row) => processFieldValue(applyCellDefault(cell, col), col.dataField, row),
                };
            }
            return col;
        });
    ...
    const renderCellContent = useCallback((row, column, rowIndex, colIndex) => {
        const rawValue = getNestedValue(row, column.dataField);
        ...
        if (isEditing) {
            ...
            if (column.editorRenderer) {
                ...
                const formatterContent = column.formatter
                    ? column.formatter(rawValue, row, rowIndex, colIndex)
        ...
        if (column.formatter) {
            return column.formatter(rawValue, row, rowIndex, colIndex);
        }
        const displayValue = applyCellDefault(rawValue, column);
    ...
    const renderToolbar = () => {
        ...
        if (typeof toolbar === 'function') {
            return toolbar({
                searchValue: clickSearchApply ? searchDraft : searchValue,
                onSearch: clickSearchApply
                    ? (v) => setSearchDraft(v)
                    : handleSearchChange,
                onClearFilters: handleClearFilters,
                onExport: handleExport,
                commitGlobalSearch: clickSearchApply
                    ? commitGlobalSearch
                    : undefined,
            });
        }
        ...
        const hasFilters = Object.keys(filters).length > 0 ||
            searchValue ||
            (clickSearchApply && Boolean(searchDraft));
        const clearSearchDisabled = clickSearchApply
            ? !searchDraft && !searchValue
            : !searchValue;
        return (_jsxs(Toolbar, { className: classNames.toolbar, style: styles.toolbar, children: [_jsxs(ToolbarGroup, { children: [toolbarLeft, searchable && (_jsxs(SearchInput, { children: [_jsx(SearchIcon, {}), _jsx("input", { type: "text", value: clickSearchApply ? searchDraft : searchValue, onChange: (e) => handleGlobalSearchInputChange(e.target.value), onKeyDown: (e) => {
                                        if (clickSearchApply && e.key === 'Enter') {
                                            e.preventDefault();
                                            commitGlobalSearch();
                                        }
                                    }, onFocus: () => {
    ...
    const renderPagination = () => {
        ...
        return (_jsxs(PaginationWrapper, { className: classNames.pagination, style: styles.pagination, children: [_jsxs(ToolbarGroup, { children: [(resolvedRowSelection?.mode === 'checkbox' || isSelectRow) &&
                    ...
                    })(), (resolvedRowSelection?.mode === 'checkbox' || isSelectRow) &&
                    selectedKeys.size > 0 && (_jsx(ToolbarGroup, { style: { marginLeft: 'auto' }, children: _jsxs(SelectionIndicator, { onClick: () => {
                            const selectedRows = data.filter((row) => selectedKeys.has(getRowKey(row)));
                        ...
                        }, children: [_jsx(CheckIcon, { size: 16, color: "white" }), _jsx(SelectionCount, { "$animate": selectionAnimation, children: selectedKeys.size }, selectedKeys.size)] }) }))] }));
    };
    ...
    return (_jsxs(TableRoot, { ref: (node) => {
            ...
            tableContainerRef.current = node;
                    ...
                    zIndex: 20,
                                        ...
                                        const onColumnFilter = (value) => handleFilterChange(column.dataField, value, {
                                            clickApply: columnClickFilter,
                                        });
                                        const factoryGetFilter = column.filterRenderer
                                            ? getColumnFilterGetFilterCallback(column.filter)
                                            : undefined;
                                        const columnClickFilter = column.isClickFilter ?? isClickFilter;
                                                    ...
                                                    return (_jsxs("div", { style: {
                                                            ...
                                                            zIndex: 2, // Above resize handle
                                                                    ...
                                                                    minHeight: 24,
                                                                            ...
                                                                            alignItems: 'center',
                                                                            overflow: 'hidden',
                                                                        }, onClick: (e) => e.stopPropagation(), children: _jsx(FilterClickApplyProvider, { value: columnClickFilter, children: column.filterRenderer ? (_jsxs(FilterValueProvider, { value: filters[column.dataField], children: [factoryGetFilter ? (_jsx(FilterRendererGetFilterBridge, { getFilter: factoryGetFilter, dataField: column.dataField, filters: filters, handleFilterChange: handleFilterChange })) : null, column.filterRenderer(onColumnFilter, column, {
                                                                                        value: filters[column.dataField],
                                                                                        onClear: () => handleFilterChange(column.dataField, null),
                                                                                        isClickFilter: columnClickFilter,
                                                                                ...
                                                                                }, "data-filter-wrapper": column.dataField, style: { width: '100%' }, children: _jsx(ResolvedFilter, { column: column, value: filters[column.dataField], onChange: onColumnFilter, isClickFilter: columnClickFilter, onClear: () => handleFilterChange(column.dataField, null) }, `filter-${column.dataField}`) })) }) }), showSort && (_jsx("div", { style: {
                                                                            minWidth: 40,
                                                                            minHeight: 24,
                                                                            display: 'flex',
                                                                            alignItems: 'center',
