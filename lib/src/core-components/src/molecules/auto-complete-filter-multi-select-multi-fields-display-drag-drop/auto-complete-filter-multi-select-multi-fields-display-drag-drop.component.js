...
export const AutoCompleteFilterMultiSelectMultiFieldsDisplayDragDrop = ({ key, uniqueField = '_id', loader = false, placeholder = 'Search...', data, hasError = false, disable = false, isUpperCase = false, name, defaultValue = '', onFilter, onUpdate, onSelect, onBlur, }) => {
    ...
    return (_jsx(_Fragment, { children: _jsxs("div", { ref: wrapperRef, className: cn(s['w-full'], s['relative']), children: [_jsxs("div", { className: cn(s['flex'], s['items-center'], s['leading-4'], s['p-1'], s['focus:outline-none'], s['focus:ring'], s['w-full'], s['shadow-sm'], s['sm:text-base'], s['border-1'], hasError ? s['border-red'] : s['border-gray-300'], s['rounded-md']), children: [_jsx("input", { placeholder: placeholder, disabled: disable, name: name, value: !isListOpen
                    ...
                    ? options?.length > 0 && (_jsxs("div", { className: cn(s['flex'], s['flex-col'], s['mt-1'], s['absolute'], s['rounded-sm'], s['z-500'], s['w-full']), style: { zIndex: 80 }, children: [_jsx("div", { className: cn(s['flex'], s['w-full'], s['bg-gray-400']), children: _jsx(DndContext, { sensors: sensors, collisionDetection: closestCenter, onDragEnd: onDragEnd, children: selectedItems?.length > 0 && (_jsx(SortableContext, { items: selectedItems.map((item) => item.sort), strategy: horizontalListSortingStrategy, children: _jsx("div", { className: cn(s['flex'], s['flex-wrap'], s['gap-1'], s['bg-gray-400'], s['p-2']), children: selectedItems?.map((item, index) => (_jsx(DraggableSelectedItem, { item: item, index: index, onRemove: handleRemoveItem, displayKey: data.displayKey, uniqueField: "sort" }, item.sort))) }) })) }) }), _jsx("ul", { className: cn(s['flex'], s['flex-col'], s['gap-2'], s['bg-gray-200'], s['p-2'], s['w-full']), id: `${key}_listView`, children: options?.map((item, index) => (_jsx(_Fragment, { children: _jsx("li", { className: cn(s['flex'], s['gap-4'], s['p-1']), children: _jsxs("label", { className: cn(s['flex'], s['items-center']), children: [_jsx("input", { className: cn(s['flex'], s['h-4'], s['w-4']), type: "checkbox", checked: item.selected, onChange: () => onSelect(item), id: `${key}_${index}_check_box_view` }, `${key}_${index}_check_box_view`), ' ', _jsx("span", { className: cn(s['flex'], s['ml-2']), children: data.displayKey
                                                        .map((key) => `${item[key]}
                              `)
